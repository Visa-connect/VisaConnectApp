...
const MeetupsInterestedScreen: React.FC = () => {
  ...
  useEffect(() => {
    fetchInterestedMeetups();
  }, []);

  const fetchInterestedMeetups = async () => {
    try {
      setLoading(true);
      setError(null);
      const interestedMeetups = await meetupService.getUserInterestedMeetups();
      setMeetups(interestedMeetups);
    } catch (err) {
      console.error('Error fetching interested meetups:', err);
      setError('Failed to load your interested meetups. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">
            Meetups I'm Interested In
          </h1>
          <p className="text-gray-600 mt-2">
            View all the meetups you've expressed interest in
          </p>
        </div>
        ...
        <div className="space-y-6">
          {loading && (
            <div className="text-center py-8">
              <div className="text-gray-500">
                Loading your interested meetups...
              </div>
            </div>
          )}
          ...
          {error && (
            <div className="text-center py-8">
              ...
              <Button variant="secondary" onClick={fetchInterestedMeetups}>
                Try Again
              </Button>
            </div>
          )}
          ...
          {!loading && !error && meetups.length === 0 && (
            <div className="text-center py-8">
              <div className="text-gray-500 mb-4">
                You haven't expressed interest in any meetups yet.
              </div>
              <Button variant="primary" onClick={() => navigate('/meetups')}>
                Browse Meetups
              </Button>
            </div>
          )}
...
export default MeetupsInterestedScreen;
